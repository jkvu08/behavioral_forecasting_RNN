...
def hyp_ende_nest(params, features, targets):
    ...
    if params['mtype']=='LSTM': # if the model is an LSTM
        model.add(LSTM(units =params['neurons_n0'], input_shape = (lookback,features), return_sequences= params['architecture']['return_seq'], name = 'LSTM')) # set the RNN type
        ...
        model.add(GRU(units =params['neurons_n0'], input_shape = (lookback,features), return_sequences= params['architecture']['return_seq'])) # set the RNN type
        model.add(Dropout(rate= params['drate'])) # add another drop out
    ...
    model.compile(loss = 'categorical_crossentropy', optimizer = Adam(learning_rate = params['learning_rate']), sample_weight_mode = 'temporal') # compile the model
    return model 
...
def hyperoptimizer_ende(params):
    ...
    _, f1_value,val_loss,train_f1, train_loss = eval_f1_iter(model, params, train_X, train_y, test_X, test_y, patience = 30, atype ='ENDE', max_epochs = 100, n=1) # fit model and extract monitoring metrics
...
space_ende = {'drate'                 : hp.uniform('drate',0.1,0.9),
              'neurons_n0'             : scope.int(hp.quniform('neurons_n0',1,100,1)),
              'neurons_n1'             : scope.int(hp.quniform('neurons_n1',1,100,1)),
              'n_output'               : scope.int(hp.quniform('n_output',1,6,1)),
              'td_neurons'             : scope.int(hp.quniform('td_neurons',1,100,5)),
              ...
              'epochs'                 : 0,
              'batch_size'             : scope.int(hp.quniform('batch_size',144,1024,12)),
              'weights_0'              : hp.uniform('weights_0',0.0001,100),
              'weights_1'              : hp.uniform('weights_1',0.0001,100),
              'weights_2'              : hp.uniform('weights_2',0.0001,100),
              'weights_3'              : hp.uniform('weights_3',0.0001,100),
...
# assign the fitting objective, the algorithm , the number of trials, and save each trial
best = fmin(hyperoptimizer_ende, space_ende, algo=tpe.suggest, max_evals = 25, trials = trials, rstate = np.random.RandomState(154)) # output the hyperparameter set for the best model (min losses)
...
g = 1
while g < 1000:
    run_trials(filename = 'ende_rnn_trials.pkl',objective =hyperoptimizer_ende, space =space_ende, rstate =90, initial=25, trials_step=3)
    g += 1

# ENDE
# load hyperopt trials
ende_1 = joblib.load('ende_rnn_trials_seed1.pkl')
ende_2 = joblib.load('ende_rnn_trials_seed2.pkl')
ende_3 = joblib.load('ende_rnn_trials_seed3.pkl')

# plot trial results
ende_dict = hopt_comp([1,2,3], 'ende_rnn_trials_seed','ende')
ende_sub = sub_comp(ende_dict)
kde_comp([4,5,6,7,8],ende_sub,0.45)
kde_comp([10,11,12,13,14],ende_sub,0.45)
kde_comp([15,16,17,18,19],ende_sub,0.45)
kde_comp([20,22,23,24],ende_sub,0.45)